...
import { 
  ...
  ShieldCheck, Check, AlertCircle, ChevronDown, X
...
export default function SignupScreen({ navigation }) {
  ...
  const handleRegister = async (isSkipping = false) => {
    if (!shopName || !slug || !phone || !password) return Toast.show({ type: 'error', text1: 'Please fill all basic fields' });
    ...
    try {
        ...
        if (err.response && err.response.status === 409) {
            return Toast.show({ type: 'error', text1: 'Phone Number Taken', text2: 'This number is already registered. Please Sign In.' });
        } else {
            return Toast.show({ type: 'error', text1: 'Connection Error', text2: 'Unable to verify phone number.' });
        }
    }
        ...
        >
          ...
          <View style={styles.formBox}>
              ...
              <View style={styles.inputGroup}>
                  ...
                  <View style={styles.inputContainer}>
                      <LinkIcon size={20} color="#94a3b8" style={styles.icon}/>
...
const styles = StyleSheet.create({
  ...
  inputContainer: { backgroundColor: '#f8fafc', borderColor: '#f1f5f9', borderWidth: 2, borderRadius: 16, flexDirection: 'row', alignItems: 'center', paddingHorizontal: 16, height: 64 },
  ...
  divider: { height: 1, backgroundColor: '#f1f5f9', width: '100%', marginVertical: 24 },
